...
function getCitiesData($conn, $get_deleted = true)
{
    ...
    foreach ($cities as &$city) {
        $sqli = "SELECT spot.name, spot.description, spot.id_city, spot.deleted as spotdeleted 
                 FROM spot 
                 INNER JOIN city 
                     ON spot.id_city=city.id 
                 WHERE city.id='" . $city->id . "' AND spot.deleted != 1";
        $result = mysqli_query($conn, $sqli);
...
function getActiveMenuLink($link)
{
    ...
    return strpos($url, $link) ? ' active-link-menu' : '';
}